...
async function check() {
  try {
    ...
    console.log("Connected to DB:", dbName.recordset[0].db);
    
    const result = await pool.request()
        .query("SELECT id, status, doc_name FROM documents WHERE doc_name LIKE '%Minutes of Management Review Meeting%'");
